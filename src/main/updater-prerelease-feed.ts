...
import {
  ...
  getUpdateReleaseTagHrefPattern,
  getUpdateReleasesDownloadBase
} from '../shared/updater-feed'
...
export function getReleaseDownloadUrl(tag: string): string {
  return `${getUpdateReleasesDownloadBase()}/${encodeURIComponent(tag)}`
}
...
async function fetchReleaseFeedTags(): Promise<ReleaseFeedTag[] | null> {
  try {
    const res = await net.fetch(getUpdateAtomFeedUrl(), {
    ...
    for (const match of body.matchAll(getUpdateReleaseTagHrefPattern())) {
      const tag = match[1]
      const version = normalizeTagToVersion(tag)
